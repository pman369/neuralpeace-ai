...
export default function BrainAtlas({ highlightRegion }: BrainAtlasProps) {
  ...
  const parts = useMemo(
    () => [
      {
        name: 'Frontal' as BrainRegion,
        pos: [0, 0.2, 0.8] as [number, number, number],
        scale: [0.8, 0.7, 0.7],
        color: '#64b5f6',
      },
      {
        name: 'Parietal' as BrainRegion,
        pos: [0, 0.5, -0.2] as [number, number, number],
        scale: [0.75, 0.6, 0.7],
        color: '#81c784',
      },
      {
        name: 'Temporal' as BrainRegion,
        pos: [0.7, -0.1, 0.2] as [number, number, number],
        scale: [0.4, 0.5, 0.6],
        color: '#ffd54f',
      },
      {
      ...
      {
        name: 'Occipital' as BrainRegion,
        pos: [0, 0.1, -0.9] as [number, number, number],
        scale: [0.6, 0.5, 0.5],
        color: '#ba68c8',
      },
      {
        name: 'Cerebellum' as BrainRegion,
        pos: [0, -0.6, -0.7] as [number, number, number],
        scale: [0.5, 0.4, 0.4],
        color: '#e57373',
      },
    ],
  ...
  return (
    <div className="w-full h-full bg-surface-container-lowest rounded-3xl overflow-hidden relative border border-outline-variant/10 shadow-inner">
      ...
      <div className="absolute top-6 left-6 z-10 pointer-events-none">
        <h3 className="text-xs font-bold text-outline uppercase tracking-widest mb-1">
          Anatomical Model
        </h3>
        <div className="flex items-center gap-2">
          <div
            className={`w-2 h-2 rounded-full ${activeRegion ? 'bg-primary animate-pulse' : 'bg-outline/30'}`}
          />
